...
def create_auth_file():
    # with open("authd.pass.jinja2") as file_:
    #     template = Template(file_.read())
    #     config = template.render(
    #         agent_registration_password=agent_registration_password
    #     )
    wazuh_authd_file = open("/etc/authd.pass", "w")
    wazuh_authd_file.write(f"{agent_registration_password}\n")
    wazuh_authd_file.close()
...
def restart_ossec():
    ...
    std_out, std_err, r_code = execute([cmd, "start"])
    restarted = False
...
if __name__ == "__main__":
    try:
        ...
        join_manager_port = os.environ.get("JOIN_MANAGER_PORT") # agent registration port
        agent_registration_password = os.environ.get("AGENT_REGISTRATION_PASSWORD")
        ...
        join_manager_worker = os.environ.get("JOIN_MANAGER_WORKER_HOST") # # agent registration server
